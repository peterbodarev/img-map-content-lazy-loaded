...
function lazyScroll() {
	...
	if (!loadMoreBlock.classList.add('_loading')) {
		loadMore();
	}
}
	...
	if (pageYOffset > loadMapBlockPos - windowHeight) {
		...
		if (loadMapUrl) {
			loadMapBlock.insertAdjacentHTML(
				'beforeend',
				`<iframe src="${loadMapUrl}" style="border: 0;" allowfullscreen='' loading="lazy"></iframe></iframe>`
			);
...
async function getContent() {
	...
	if (response.ok) {
		...
		let incrementResultNr = result.replace(
			'( <span></span> )',
			`( <span>${loadedParagraphNumber++}</span> )`
		);
		loadMoreBlock.insertAdjacentHTML('beforeend', incrementResultNr);
